...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="password" class="block text-sm font-semibold text-slate-700 mb-1">Password (kosongkan jika tidak ingin diubah)</label>
        ...
        <div>
            ...
            <select name="role" id="role" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 focus:border-indigo-500 outline-none">
                <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>User Pembantu (Kelola Konten)</option>
