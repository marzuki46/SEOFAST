...
class InternalLinkController extends Controller
{
    ...
    public function generateAi(Request $request)
    {
        ...
        // Prevent timeout during batch AI processing
        set_time_limit(0);
        ...
        foreach ($chunks as $chunk) {
            ...
            if (is_array($aiAnchors) && count($aiAnchors) === count($chunk)) {
                $finalAiAnchors = array_merge($finalAiAnchors, $aiAnchors);
            } else {
                $totalSuccess = false;
                // Fill the failed chunk with nulls so the index aligns
        ...
        foreach ($plannedLinks as $idx => $link) {
            $aiText = $finalAiAnchors[$idx] ?? null;
            // Fallback to exact target keyword if AI fails for this specific chunk
            $anchorText = $aiText ? $aiText : $link['target']->target_keyword;
        ...
        if (!$totalSuccess) {
            return redirect()->back()->with('error', 'AI partially or fully failed to generate anchors. Fallback applied to some links. Please check AI configuration or API limits.');
        }

        return redirect()->back()->with('success', 'High-CTR Anchor Texts successfully generated by AI in batches (max 3 links per article)!');
    }
}
