...
<?php $__env->startSection('title', $resolvedTitle); ?>
<?php $__env->startSection('meta_description', $postMeta['description'] ?? ''); ?>
<?php $__env->startSection('canonical_url', $postMeta['canonical']); ?>
<?php if(isset($isPreview) && $isPreview): ?>
    <?php $__env->startSection('robots_meta', 'noindex, nofollow'); ?>
<?php else: ?>
    <?php $__env->startSection('robots_meta', $postMeta['robots']); ?>
<?php endif; ?>
<?php $__env->startSection('og_image', $postMeta['og_image']); ?>
<?php $__env->startSection('og_title', $postMeta['og_title'] ?? $resolvedTitle); ?>
<?php $__env->startSection('og_description', $postMeta['og_desc'] ?? ($postMeta['description'] ?? '')); ?>
...
<?php $__env->startSection('schema_markup'); ?>
    <?php echo \App\Services\SeoHelper::renderSchema($post); ?>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <nav class="flex items-center gap-2 text-xs text-slate-500 font-medium">
            ...
            <?php if($category): ?>
                ...
                <a href="<?php echo e(route('blog.category', $category->slug)); ?>" class="hover:text-slate-900 transition-colors"><?php echo e($category->silo_name); ?></a>
            <?php endif; ?>
            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
            <span class="text-slate-800 truncate max-w-[200px] md:max-w-xs font-semibold"><?php echo e($post->title); ?></span>
        </nav>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
            ...
            <div class="lg:col-span-3 space-y-8">
                ...
                <header class="space-y-6">
                    <div class="flex flex-wrap items-center gap-4 text-sm">
                        <?php if($category): ?>
                            <a href="<?php echo e(route('blog.category', $category->slug)); ?>" class="px-3 py-1 rounded-full bg-brand-indigo/10 border border-brand-indigo/20 text-brand-indigo font-bold text-xs uppercase">
                                <?php echo e($category->silo_name); ?>

                            </a>
                        <?php endif; ?>
                        <span class="text-slate-300">•</span>
                        <span class="text-slate-600 font-medium">Last Updated: <?php echo e($post->updated_at ? $post->updated_at->format('F d, Y') : ($post->published_at ? $post->published_at->format('F d, Y') : '')); ?></span>
                    </div>
                ...
                <div class="pt-8 mt-12 border-t border-slate-200 flex items-center gap-4">
                    ...
                    <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo e(urlencode(request()->url())); ?>" aria-label="Share on Facebook" target="_blank" rel="noopener noreferrer" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-[#1877F2] hover:text-white transition-all">
                    ...
                    <a href="https://twitter.com/intent/tweet?url=<?php echo e(urlencode(request()->url())); ?>&text=<?php echo e(urlencode($post->title)); ?>" aria-label="Share on Twitter" target="_blank" rel="noopener noreferrer" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-[#1DA1F2] hover:text-white transition-all">
                    ...
                    <a href="https://api.whatsapp.com/send?text=<?php echo e(urlencode($post->title . ' - ' . request()->url())); ?>" aria-label="Share on WhatsApp" target="_blank" rel="noopener noreferrer" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-[#25D366] hover:text-white transition-all">
                                   ...
                                   class="p-5 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-md transition-all flex flex-col justify-between group">
                                    <div>
                                        <span class="text-[10px] text-slate-400 font-mono uppercase mb-2 block">
                                            <?php echo e($rel->published_at ? $rel->published_at->format('M d, Y') : ''); ?>
            ...
            <div class="space-y-8">
                ...
                <div class="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm sticky top-28">
                    ...
                    <nav id="toc" class="space-y-2 text-sm text-slate-600">
                        ...
                        <ul class="space-y-2 border-l border-slate-200 pl-4" id="toc-list">
                            <?php $tocItems = $post->toc; ?>
                            <?php if(count($tocItems) > 0): ?>
                                <?php $__currentLoopData = $tocItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $heading): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li class="<?php echo e($heading['level'] === 3 ? 'pl-4 text-xs' : 'text-sm font-medium'); ?>">
                                        <a href="#<?php echo e($heading['id']); ?>" class="hover:text-brand-indigo transition-colors block">
                                            <?php echo e($heading['title']); ?>
                ...
                <div class="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <h3 class="font-outfit font-bold text-slate-900 mb-4 text-xs tracking-wider uppercase">Also in <?php echo e($category ? $category->silo_name : 'this category'); ?></h3>
                    <div class="space-y-4">
                        <?php $__currentLoopData = $relatedPosts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recent): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.frontend', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Program Marzuki\Projek Framework\SEOFAST\resources\views/blog/show.blade.php ENDPATH**/ ?>