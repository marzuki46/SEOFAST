    ...
    public function up(): void
    {
        ...
        Schema::table('products', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('products', 'purchase_count')) {
                $table->integer('purchase_count')->default(0)->after('is_featured');
            }
        });

        // ── 2. Mark SEOFAST Framework as featured + purchase count ──
        $seofast = Product::where('slug', 'seofast-framework')->first();
        if ($seofast) {
            $seofast->update([
                'is_featured' => true,
                'purchase_count' => 47,
        ...
        // also give Pro SEO Tool some purchase count
        $proSeo = Product::where('slug', 'pro-seo-tool')->first();
        if ($proSeo) {
            $proSeo->update([
                'purchase_count' => 12,
            ]);
        ...
        // Content id=10 has body_raw but status=failed_cqi → publish it
        $content10 = Content::find(10);
        if ($content10 && $content10->status !== 'published') {
            $content10->update([
                'status' => 'published',
                'published_at' => $content10->published_at ?? now()->subDays(2),
            ]);
        }

        // Content id=27 is draft with body_raw → publish it
        $content27 = Content::find(27);
        if ($content27 && $content27->status !== 'published') {
            $content27->update([
                'status' => 'published',
                'published_at' => $content27->published_at ?? now()->subDay(),
            ]);
        }
...
<section class="mb-24 anim-fade-up" id="layanan">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 anim-stagger">
        <div class="gradient-top bg-white rounded-2xl border border-slate-200 p-8 card-lift shadow-sm hover:shadow-xl">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-6 mx-auto" style="background:linear-gradient(135deg,#6366f1,#9333ea);box-shadow:0 10px 15px -3px rgba(99,102,241,0.2)">
            ...
            <p class="text-slate-500 text-sm leading-relaxed mb-4">
                Lisensi dan paket produk digital siap pakai: <strong>SEOFAST Framework</strong>, Pro SEO Tool, AI tools, dan template Laravel. Cocok untuk developer, agency, dan bisnis.
            </p>
        ...
        <div class="gradient-top bg-white rounded-2xl border border-slate-200 p-8 card-lift shadow-sm hover:shadow-xl">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-6 mx-auto" style="background:linear-gradient(135deg,#10b981,#059669);box-shadow:0 10px 15px -3px rgba(16,185,129,0.2)">
            ...
            <p class="text-slate-500 text-sm leading-relaxed mb-4">
                Done-for-you SEO service. Dari audit teknis, riset keyword, silo structure, optimasi on-page, sampai monitoring ranking. Cocok untuk bisnis yang ingin naik peringkat.
            </p>
            <div class="flex flex-wrap gap-2">
        ...
        <div class="gradient-top bg-white rounded-2xl border border-slate-200 p-8 card-lift shadow-sm hover:shadow-xl">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-6 mx-auto" style="background:linear-gradient(135deg,#a855f7,#db2777);box-shadow:0 10px 15px -3px rgba(168,85,247,0.2)">
            ...
            <p class="text-slate-500 text-sm leading-relaxed mb-4">
                Layanan pembuatan konten AI berkualitas tinggi, pipeline automation, dan project AI custom. Multi-agent system — bukan asal generate.
            </p>
            <div class="flex flex-wrap gap-2">
...
<!-- ════════════════════════════════════════════════════════ -->
<!-- PRODUK UNGGULAN — Spotlight                              -->
<!-- ════════════════════════════════════════════════════════ -->
<section class="mb-24 anim-scale-in" id="produk-unggulan">
    <div class="text-center max-w-3xl mx-auto mb-12">
        <span class="inline-block text-xs font-bold text-amber-600 uppercase tracking-[0.2em] mb-4">BEST SELLER</span>
        <h2 class="font-outfit font-bold text-4xl md:text-5xl text-slate-900 mb-4 leading-tight">
            Produk Unggulan
        </h2>
        <p class="text-slate-500 text-lg leading-relaxed">
            Paling banyak dicari dan digunakan oleh developer serta agency di Indonesia.
        </p>
    </div>
    <div class="seofast-featured-product"></div>
</section>

<!-- ════════════════════════════════════════════════════════ -->
<!-- KATALOG PRODUK DIGITAL — Semua Produk                    -->
<!-- ════════════════════════════════════════════════════════ -->
<section class="mb-24 anim-fade-up" id="produk">
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div>
            ...
            <h2 class="font-outfit font-bold text-3xl md:text-5xl text-slate-900 mb-2 leading-tight">
                Semua Produk Digital
            </h2>
            <p class="text-slate-500">Framework, tools, dan template siap pakai untuk project digital kamu.</p>
        </div>
        <a href="{$produkUrl}" class="inline-flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors group shrink-0">
            Lihat Semua Produk
    ...
    <div class="seofast-products-grid anim-stagger" data-columns="3" data-limit="10"></div>
</section>
...
<section class="mb-24 anim-fade-up" id="blog">
    <div class="text-center max-w-3xl mx-auto mb-14">
        ...
        <p class="text-slate-500 text-lg leading-relaxed">
            Catatan seputar SEO, Laravel, AI, dan digital marketing dari pengalaman sehari-hari.
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="flex items-center justify-between mb-6">
                <h3 class="font-outfit font-bold text-xl text-slate-900 flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-indigo-600"></span>
                    Terbaru
                </h3>
                <a href="{$blogUrl}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">Lihat Semua →</a>
            </div>
        ...
        <div>
            ...
            <div class="seofast-hot-posts" data-limit="5"></div>
        </div>
    </div>
</section>
...
HTML;
        ...
        $page->update([
            ...
            'hero_subheadline' => 'Juki Digital Marketing — jual SEOFAST Framework, Pro SEO Tool, layanan SEO done-for-you, dan jasa AI content & otomasi. B2B oriented, transparan, dan terukur.',
            'meta_title' => 'Juki Digital Marketing — Produk Digital, Jasa SEO & AI untuk B2B',
            'meta_description' => 'Juki Digital Marketing: jual SEOFAST Framework Laravel, Pro SEO Tool, jasa SEO, dan jasa AI untuk bisnis. B2B. Dari Silo Architecture sampai AI content pipeline.',
            'hero_cta_text' => 'Konsultasi Gratis',
            'hero_cta_url' => $contactUrl,
            'hero_cta_text_2' => 'Lihat Produk',
            'hero_cta_url_2' => '#produk-unggulan',
            'hero_features' => [
                'Produk Digital',
                'SEO Service',
                'AI Service',
    ...
    {
        Schema::table('products', function (Blueprint $table) {
            $table->dropColumn(['is_featured', 'purchase_count']);
        });
    }
};
